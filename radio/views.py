...
class ListStationsView(generic.ListView):
    ...
    def get_queryset(self):
        station_ids = Listener.objects.filter(user=self.request.user).values_list(
...
class CreateStationView(CreateView):
    ...
    def form_valid(self, form):
        ...
        Listener.objects.create(
            user=self.request.user, station=self.object, is_admin=True, is_dj=True
        )

        return response


class DetailStationView(
    LoginRequiredMixin,
    ListenerRequiredMixin,
...
class DeleteStationView(LoginRequiredMixin, ListenerRequiredMixin, DeleteView):
    ...
    def post(self, request: HttpRequest, *args, **kwargs):
        ...
        listener = Listener.objects.get(user=request.user, station_id=kwargs["pk"])
...
def oauth_callback(request: HttpRequest):
    ...
    if "error" in result:
        ...
        code = result["code"]
        spotify.AccessToken.request_refresh_and_access_token(code, request.user)

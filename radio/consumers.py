...
def station_join_required(func):
    def wrap(self, *args, **kwargs):
        if self.state != StationState.Connected:
            raise ClientError("bad_request", "user has not connected to station")
        else:
...
def station_admin_required(func):
    def wrap(self, *args, **kwargs):
        if not self.is_admin:
            raise ClientError(
                "forbidden", "user does not have permission to request listeners"
            )
        else:
            return func(self, *args, **kwargs)
...
class StationConsumer(AsyncJsonWebsocketConsumer):
    ...
    async def join_station(self):
        ...
        if self.is_admin:
            await self.channel_layer.group_add(
                station.admin_group_name, self.channel_name
            )

        # Message admins that a user has joined the station
        await self.admin_group_send_join(
            station.admin_group_name, self.user.username, self.user.email
        )
    ...
    async def leave_station(self):
        station = await get_station_or_error(self.station_id, self.user)
        await self.admin_group_send_leave(
            station.admin_group_name, self.user.username, self.user.email
        )
    ...
    async def send_playback_state_changed(self, playback_state):
        ...
        await self.send_json(
            {"type": "playback_state_changed", "playbackstate": serializer.data,}
        )

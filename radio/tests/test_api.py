...
class StationTests(APITestCase):
    def setUp(self):
        ...
        assert self.client.login(username=self.user1.username, password=password)
    ...
    def test_can_update_stations(self):
        ...
        response = self.client.patch(
            ...
            data={"playbackstate": {"raw_position_ms": 1,},},
            format="json",
        )
        assert response.status_code == HTTPStatus.OK
        assert PlaybackState.objects.get(station_id=station.id).raw_position_ms == 1
...
class ListenerTests(APITestCase):
    ...
    def test_can_get_listeners(self):
        user2 = create_user2()
        listener2 = create_listener(self.station, user2, is_admin=False, is_dj=True)

        response = self.client.get(f"/api/v1/stations/{self.station.id}/listeners/")
        ...
        actual_data = {listener["id"]: dict(listener) for listener in response.data}
    ...
    def test_can_only_get_listeners_if_authorized(self):
        ...
        # user1 is not a listener of station2
        response = self.client.get(f"/api/v1/stations/{station2.id}/listeners/")
        assert response.status_code == HTTPStatus.NOT_FOUND.value
...
class AccessTokenTests(APITestCase):
    ...
    def test_cannot_refresh_someone_elses_access_token(self):
        ...
        response = self.client.post(f"/api/v1/users/{user2.id}/accesstoken/refresh/")
        assert response.status_code == HTTPStatus.FORBIDDEN.value


def create_user1(password: str) -> User:
    return auth.get_user_model().objects.create_user(
        username=MOCK_USERNAME1, email="testuser1@example.com", password=password
    )
...
def create_playback_state(
    station: Station, paused=True, raw_position_ms=0
) -> PlaybackState:

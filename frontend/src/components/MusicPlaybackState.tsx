...
export class MusicPlaybackStatePosition extends React.Component<
  IMusicPlaybackStatePositionProps,
  IMusicPlaybackStatePositionState
> {
  ...
  public render(): JSX.Element {
    return (
      <div>
        {msToTimeString(this.state.autoPosition)}/
        {msToTimeString(this.props.duration)}
      </div>
    );
  }

  private ensureEnableRefresh(): void {
    this.refreshTimeoutId = window.setInterval(() => {
      this.setState((state) => ({
        autoPosition:
          state.autoPosition + MUSIC_POSITION_VIEW_REFRESH_INTERVAL_MS,
      }));
    }, MUSIC_POSITION_VIEW_REFRESH_INTERVAL_MS);
  }

  private ensureDisableRefresh(): void {
...
function msToTimeString(ms: number): string {
  ...
  const secondsRemainderPad =
    secondsRemainder.length === 1 ? "0" + secondsRemainder : secondsRemainder;
...
export function MusicPlaybackState(
  props: IMusicPlaybackStateProps
): JSX.Element {
  return (
    <div>
      <div>
      ...
      {props.playbackState.current_track_name}
      <br />
      {props.playbackState.artist_name}
      <br />
...
function makeMusicPlaybackStatePositionKey(
  playbackState: PlaybackState
): string {
  if (playbackState) {
    ...
    return (
      `${playbackState.context_uri}-${playbackState.current_track_uri}-` +

...
export class StationManager extends React.Component<
  IStationManagerProps,
  IStationManagerState
> {
  constructor(props: IStationManagerProps) {
    ...
    this.onPreviousTrackButtonClick = this.onPreviousTrackButtonClick.bind(
      this
    );
  ...
  public getAdjustedPlaybackPosition(serverState: PlaybackState): number {
    ...
    if (!serverState.paused) {
      ...
      adjustment =
        new Date().getTime() -
        (serverState.sample_time.getTime() - serverTimeOffset);
    }
  ...
  private bindSteadyStateActions() {
    this.state.musicPlayer.on("player_state_changed", (clientState) => {
      if (clientState) {
        if (
          this.state.clientEtag &&
          clientState.sample_time <= this.state.clientEtag
        ) {
          return;
        }
    ...
    this.props.server.on(
      "playback_state_changed",
      (serverState: PlaybackState) => {
        this.state.taskExecutor.push(() =>
          this.applyServerPlaybackState(serverState)
        );
      }
    );
  }
  ...
  private async updateServerPlaybackState(
    ...
    if (
      !playbackState ||
      (this.state.clientEtag &&
        playbackState.sample_time <= this.state.clientEtag)
    ) {
      return;
    }

    // Convert sample time from client time to server time
    playbackState.sample_time = new Date(
      playbackState.sample_time.getTime() +
        this.getMedianClientServerTimeOffset()
    );

    try {
      const serverState: PlaybackState = await Promise.race([
        this.props.server.sendPlaybackState(playbackState),
        timeout(5000),
      ]);
  ...
  private async syncServerPlaybackState(): Promise<void> {
    const serverState = await Promise.race([
      this.props.server.getPlaybackState(),
      timeout(5000),
    ]);
    if (serverState) {
  ...
  private async calculatePing(): Promise<void> {
    const pong = await Promise.race([
      this.props.server.sendPingRequest(),
      timeout(5000),
    ]);
  ...
  private async refreshOAuthToken(): Promise<string> {
    const response = await Promise.race([
      this.props.server.refreshOAuthToken(this.props.userId),
      timeout(5000),
    ]);
    this.setState({
  ...
  private async applyServerPlaybackState(
    serverState: PlaybackState
  ): Promise<void> {
    ...
    try {
      ...
      if (
        !clientState ||
        clientState.context_uri !== serverState.context_uri ||
        clientState.current_track_uri !== serverState.current_track_uri
      ) {
        await this.state.musicPlayer.play(
          serverState.context_uri,
          serverState.current_track_uri
        );
      }

      await Promise.race([
        retry(() => this.currentTrackReady(serverState)),
        timeout(5000),
      ]);
      ...
      if (serverState.paused) {
        ...
        if (Math.abs(localPosition - serverPosition) > MAX_SEEK_ERROR_MS) {
          ...
          console.log(
            `Playback adjustment needed: local: ${localPosition}, server: ${serverPosition}, new local: ${newLocalPosition}`
          );
          await this.state.musicPlayer.seek(newLocalPosition);
          await Promise.race([
            retry(() => this.currentPositionReady(newLocalPosition)),
            timeout(5000),
          ]);
          const serverPositionAfterSeek = this.getAdjustedPlaybackPosition(
            serverState
          );
          if (
            newLocalPosition > serverPositionAfterSeek &&
            newLocalPosition < serverPositionAfterSeek + MAX_SEEK_ERROR_MS
          ) {
            await this.state.musicPlayer.freeze(
              localPosition - serverPositionAfterSeek
            );
          } else {
  ...
  private async showListeners(): Promise<void> {
    const listeners = await Promise.race([
      this.props.server.getListeners(),
      timeout(5000),
    ]);
  ...
  private async inviteListener(username: string): Promise<void> {
    try {
      const listener = await Promise.race([
        this.props.server.inviteListener(username, false, false),
        timeout(5000),
      ]);
      ...
      const result =
        e instanceof ListenerAlreadyExistsError
          ? ServerError.ListenerAlreadyExistsError
          : ServerError.Unknown;
      this.setState({
  ...
  private async deleteListener(listenerId: number): Promise<void> {
    try {
      await Promise.race([
        this.props.server.deleteListener(listenerId),
        timeout(5000),
      ]);
      this.setState((prevState) => ({
        listeners: prevState.listeners.filter(
          (listener: IListener) => listener.id !== listenerId
        ),
      }));
    } catch (e) {
      this.setState({
  ...
  private async currentTrackReady(
  ...
  private async currentPositionReady(
  ...
  private adjustServerTimeOffset(
    startTime: Date,
    serverTime: Date,
    currentTime: Date
  ): void {
    ...
    const clientServerTimeOffset =
      serverTime.getTime() + medianOneWayTime - currentTime.getTime();
...
export class StationServer {
  ...
  constructor(
    ...
    private webSocketBridge: IWebSocketBridge
  ) {
  ...
  public bindWebSocketBridgeActions(): void {
  ...
  // eslint-disable-next-line @typescript-eslint/explicit-module-boundary-types
  public onOnce(eventName: string, cb: any): void {
  ...
  // eslint-disable-next-line @typescript-eslint/explicit-module-boundary-types
  public removeListener(eventName: string, cb?: any): void {
    if (cb) {
  ...
  public async sendPlaybackState(
    playbackState: PlaybackState
  ...
  public async inviteListener(
    ...
    if (response.ok) {
      ...
      data.non_field_errors.some(
        (s: string) => s === "The fields user, station must make a unique set."
      )
    ) {
  ...
  private onMessage(action: any) {
    ...
    if (action.error) {
      this.observers
        .get("error")!
        .fire(serverErrorFromString(action.error), action.message);
    } else if (action.join) {
      this.observers.get("join")!.fire(action.join);
    } else if (action.type === "playback_state_changed") {
      const serverPlaybackState = createPlaybackStateFromServer(
...
export class StationMusicPlayer {
  public static getCachedVolume(): number {
    const value = localStorage.getItem("musicVolume");
    return value !== null ? parseFloat(value) : 0.8;
  }

  public static setCachedVolume(volume: number): void {
  ...
  public togglePlay(): Promise<void> {
    return this.musicPlayer.togglePlay();
  }
  ...
  public seek(positionMS: number): Promise<void> {
    return this.musicPlayer.seek(positionMS);
  }

  public previousTrack(): Promise<void> {
    return this.musicPlayer.previousTrack();
  }
...
export function formatListenerInviteResult(
  username: string,
  error?: ServerError
): string {
  if (!error) {
